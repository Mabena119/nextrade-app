...
<?php if(!isset($_GET['user'])){ ?>
	...
	if (!empty($_GET['success']) && is_string($_GET['success'])) {
		...
		$errorMessages = [
			...
			'member_sync_failed' => 'User saved but could not add them to app members. Check the database.',
...
?>
	...
	<div class="aura-console-head" style="margin-bottom:1.25rem;">
		<div>
			<a class="aura-back" href="users.php"><i class="ti ti-arrow-left"></i> All members</a>
			<p class="aura-kicker">Member</p>
			<h1><?php echo htmlspecialchars($mentorFullname); ?></h1>
			<p style="margin:0;color:var(--aura-muted);">Automations and access codes for this account.</p>
		</div>
	</div>
	...
	<div class="mu-shell">
		<div class="mu-shell-hd">
			<div class="mu-shell-hd-text">
				<h3>Automations</h3>
				<p>Keys shown are automation secret codes for API authentication.</p>
			</div>
		</div>
		<div class="mu-scroll">
				...
				<tbody>
					...
					<tr>
						<td colspan="5" class="mu-empty">
							...
							No automations for this mentor.
						</td>
					</tr>
					<?php else: while($i > 0) { ?>
					<tr>

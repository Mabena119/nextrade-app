...
import { normalizeLicenseKey, normalizePhoneSecret, readJsonBody } from '@/utils/input-security';
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const licence = normalizeLicenseKey(body.licence ?? body.license);
    const phoneSecret = normalizePhoneSecret(body.phone_secret);
    ...
    // degraded: clients must keep existing local licence state (network/DB unavailable).
    return Response.json({ message: 'error', degraded: 1 }, { status: 200 });
  } finally {
